...
1. DDL(Data Definition Language): 데이터 정의어
	: 데이트를 저장하기 위한 공간을 생성하고 논리적으로 정의하는 언어
...
/*반드시 기억해주세요!!!*/
show databases; -- (한줄 주석) 모든 데이터베이스 조회
...
*************************************/
SELECT EMP_ID, EMP_NAME FROM EMPLOYEE; -- 대소문자 상관 x
...
-- AS : 컬럼명 별칭 부여
-- 형식> SELECT [컬럼 as 별칭, ...] FROM [테이블명];
-- 사원 테이블 사원명, 사번, 성별, 입사일, 급여를 한글 별칭으로 조회
select emp_id as 사번, emp_name as 사원명, gender as 성별, hire_date as 입사일, salary as 급여
from employee;

-- 사원 테이블에서 id, name, gender, hdate, salsay 컬럼명으로 조회
...
-- 사원명이 홍길동인 사원을 별칭으로 조회??? ::: where 조건절에서 별칭을 컬럼명을 사용 X
select emp_id as '사원번호', emp_name as '사원명', eng_name, gender, hire_date, salary
from employee
where 사원명 ="홍길동";
...
select * from department;
select emp_id, emp_name, hire_date, salarym dept_id
from employee
where dept_id = 'stg';

-- 입사일이 2014년 8원 1일인 사원들을 조회
-- 내 코드
...
select emp_id, emp_name, eng_name, gender, hire_date, retire_date, salary, salary*20 as bonus
from employee
where retire_date is null;

-- 퇴사한 사원들의 사번, 사원명, 이메일, 폰번호, 급여를 조회
select emp_id, emp_name, email, phone, retire_date, salary
from employee
where retire_date is not null;

-- null 값을 다른 값을 대체하는 방법
...
select emp_id, emp_name, ifnull(eng_name, 'SMITH')as eng_name, hire_date, ifnull(retire_date, curdate())as retire_date, salary
from employee
where dept_id = 'mkt';
...
-- 급여를 기준으로 오름차순 정렬 asc/내림차순 정렬 desc
select *
from employee
order by salary asc;

-- 모든 사원을 급여, 상별을 기준으로 오름차순 정렬
select *
from employee
...
-- '급여' 별칭이로 order by가 가능할까요? :: 별칭으로 order by 가능
-- where 조건절 데이터 탐색> 컬럼리스트 > 정렬
select emp_id, emp_name, eng_name, gender, hire_date, retire_date, salary as 급여
from employee
where retire_date is not null
order by 급여 desc;
...
select emp_id, emp_name, eng_name, gender, hire_date as '입사일', salary as '급여'
from employee
where dept_id = "sys"
...
/******************************************
    ...
    형식> SELECT [컬럼리스트]
            ...
            ORDER BY [컬럼명, ...] ASC/DESC
*******************************************/
-- 금여가 5000 이상인 사원들을 조회, 급여를 오름차순 정렬
select *
from employee
where salary >= 5000
order by salary;
...
-- 입사일이 2015-01-01 이후이거나, 급여가 6000인 이산인 사원들을 조회
-- ~이거나, ~또는 : or - 두 개의 조건 중 하나만 만족해도 조회가능
select *
from employee
where hire_date>='2015-01-01' or salary >=6000;

-- 입사일이 2015-01-01 이후이고, 급여가 6000인 이산인 사원들을 조회
-- ~이거나, ~또는 : or - 두 개의 조건 중 하나만 만족해도 조회가능
select *
from employee
...
-- 급여가 6000 이상 8000 이상인 사원들을 모두 조회
select *
from employee
where salary>=6000 and salary<=8000
order by salary asc;

-- mkt 부서의 사원들의 사번, 사원명, 입사일, 이메일,급여, 보너스 (급여*20%)
-- 급여가 null인 사원의 보너스는 기본 50
-- 보너스가 1000 이상인 사원 조회
-- 보너스가 높은 사원 기준으로 정렬
select emp_id, emp_name, hire_date, dept_id, email, salary, ifnull(salary*0.2,50) as bonus
from  employee
...
-- 이메일 이름 중 두번때 자리에 'a'가 즐어가는 모든 사원을 조회
select *
from employee
